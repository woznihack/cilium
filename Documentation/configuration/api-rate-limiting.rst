...
================================= ========= ==========================================================================
``rate-limit``                    ``5/m``   Allowed requests per time unit in the format ``<number>/<duration>``
``rate-burst``                    ``4``     Burst of API requests allowed by rate limiter.
``min-wait-duration``             ``10ms``  Minimum wait duration each API call has to wait before being processed.
``max-wait-duration``             ``15s``   Maximum duration an API call is allowed to wait before it fails.
``estimated-processing-duration`` ``100ms`` Estimated processing duration of an average API call. Used for automatic adjustment.
``auto-adjust``                   ``true``  Enable automatic adjustment of ``rate-limit``, ``rate-burst`` and ``parallel-requests``.
``parallel-requests``             ``4``     Number of parallel API calls allowed.
``min-parallel-requests``         ``2``     Lower limit of parallel requests when auto-adjusting.
``max-parallel-requests``         ``6``     Upper limit of parallel requests when auto-adjusting.
``mean-over``                     ``10``    Number of API calls to calculate mean processing duration for auto adjustment.
``log``                           ``true``  Log an Info message for each API call processed.
``delayed-adjustment-factor``     ``0.25``  Factor for slower adjustment of ``rate-burst`` and ``parallel-requests``.
``max-adjustment-factor``         ``10.0``  Maximum factor the auto-adjusted values can deviate from the initial base values configured.
================================= ========= ==========================================================================
